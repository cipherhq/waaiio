...
### fix(P1-LOYAL-1): loyalty redemption code persistence — no silent failure

- **Root cause:** Bot loyalty redemption flow inserted a transaction with `reference_type: 'loyalty_points'` then attempted a separate `.update().order().limit(1)` to overwrite it with the redemption code. PostgREST does not support `.order()` and `.limit()` on UPDATE. The update result was not checked. Customer was shown the code but it was never persisted — staff verification impossible.
- **Fix:** Generate the redemption code BEFORE the INSERT. Include it directly as `reference_type: 'code:RW-XXXXXX'`. Check the INSERT result — if persistence fails, throw into the existing error handler (no success message sent). No separate UPDATE needed. No new RLS policies required — the flow uses service_role (bypasses RLS) and only needs INSERT, not UPDATE.
- **Concurrency:** `redeem_loyalty_points` RPC uses `FOR UPDATE` row lock. Transaction INSERT occurs after the RPC succeeds. No double-redemption risk.
- **Files:** `lib/bot/flows/loyalty.flow.ts`
- Could break: Nothing — the old UPDATE pattern was already unreliable.
