...
- **Fix:** (1) `apply_invoice_payment` RPC: loads payment from DB (validates status=success, invoice_id match, business match, positive amount — does NOT trust caller). Uses `SELECT FOR UPDATE` on invoice + `invoice_payment_applications` ledger with `UNIQUE(invoice_id, payment_id)`. Replay → `ON CONFLICT DO NOTHING`. `amount_paid` = `GREATEST(SUM(ledger), pre_existing_amount_paid)` — never decreases on migration. (2) `apply_campaign_donation` RPC: loads payment from DB (validates status, campaign_id match). Donation `pending→success` transition is the idempotency gate. No arbitrary LIMIT 1 fallback. (3) `platform_fees.payment_id` column + unconditional `UNIQUE(payment_id) WHERE payment_id IS NOT NULL` — one fee per payment, even after refund. Dropped entity-level UNIQUE on invoice/campaign. (4) `transaction_amount` uses actual payment amount. (5) Backfills historical successful invoice payments into ledger. (6) RPCs REVOKE from PUBLIC/anon/authenticated, GRANT only to service_role.
