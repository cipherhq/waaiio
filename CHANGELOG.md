...
- **`app/api/admin/launch-notify/route.ts`**: (CTO correction) Preview-first flow — GET returns readiness + confirmToken, POST requires confirmToken (single-use, expires 5min, same-admin verified, campaign-version-drift rejected). No send without explicit preview confirmation.
