...
- **`app/api/admin/launch-notify/route.ts`**: (CTO correction R2) Replaced in-memory Map with DB-backed confirmation tokens. GET creates `launch_delivery_confirmations` row (Preview — never sends). POST atomically consumes token via `consume_launch_confirmation` RPC. Rejects: missing token (400), not found, already consumed (replay), wrong admin, campaign mismatch, expired (all 403). Only consumed confirmation reaches `deliverLaunchNotifications`.
...
- **Payment link.** With the promo bypass fix, the scheduling flow reaches `initializePayment` with correct args (booking ID, deposit amount, business context, `transactionCategory: 'scheduling'`, `confirmationOrigin: 'whatsapp'`). Whether `initializePayment` itself succeeds in production is a separate question — the independently observed `[PAYMENT] initializePayment THREW` may be an independent defect not addressed by this fix.
...
- `lib/__tests__/scheduling-payment-regression.test.ts` — new: 13 tests exercising complete paid appointment path through initializePayment boundary with exact arg + URL + error-message assertions
