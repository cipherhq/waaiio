...
- **Fix:** (1) `apply_invoice_payment` RPC: loads payment from DB (validates status=success, invoice_id match, business match, positive amount — does NOT trust caller). Uses `SELECT FOR UPDATE` + `invoice_payment_applications` ledger with `UNIQUE(invoice_id, payment_id)`. Authoritative `amount_paid = legacy_amount_paid_baseline + SUM(ledger)`. (2) Legacy baseline: `invoices.legacy_amount_paid_baseline` column computed at migration time as `MAX(amount_paid - backfilled_sum, 0)`. Preserves unexplained pre-migration balance, never recalculated. (3) `apply_campaign_donation` RPC: loads payment from DB (validates status, campaign_id match, business_id match). No arbitrary LIMIT 1 fallback. (4) `platform_fees.payment_id` + unconditional `UNIQUE(payment_id) WHERE payment_id IS NOT NULL` — one fee per payment, even after refund. (5) `transaction_amount` uses actual payment amount. (6) Backfills historical successful payments into ledger. (7) RPCs REVOKE from PUBLIC/anon/authenticated, GRANT only to service_role.
