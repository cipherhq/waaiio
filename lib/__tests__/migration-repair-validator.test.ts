...
describe('Real Batch 1-5 evidence integration (Batches 1-4 repaired, Batch 5 verified)', () => {
  it('all verification batches and repair batches pass all validation rules together', () => {
    ...
    // 4 approved entries (Batch 9 active for repair after Batch 8 closeout)
    ...
    expect(counts['ALIGNED_TRACKED']).toBe(132);
    expect(counts['VERIFIED_APPLIED_UNTRACKED'] || 0).toBe(0);
    expect(counts['PENDING_PRODUCTION_REVERIFICATION'] || 0).toBe(0);
    expect(counts['NOT_VERIFIABLE_SAFELY']).toBe(12);
    expect(counts['SUPERSEDED_WITH_EQUIVALENT_STATE']).toBe(2);

    // Allowlist = 15 (Batch 8 active), Candidates = 0 (all verified)
    ...
    expect(pendingCount + verifiedCount + repairedCandidates).toBe(124);
    expect(repairedCandidates).toBe(124); // 15*8 + 4 batch 9
    ...
    expect(allRepairVersions.length).toBe(124); // 15 * 8

    // Validate each repair batch
    for (const repair of repairs) {
      expect(repair.approved_versions.length).toBeGreaterThanOrEqual(4); // B9 has 4, others 15
    ...
    // Batch 8 uses whole-file SHA (same as Batches 6-7) — validate its entries with the file digest
    const b8RepairPath = resolve(evidenceDir, 'batch-08-repair.json');
    if (existsSync(b8RepairPath)) {
      const b8RepairDigest = createHash('sha256').update(readFileSync(b8RepairPath)).digest('hex');
      const b8CompletedEntries = manifest.filter(e => e.repair_batch === 8 && e.repair_status === 'completed');
      expect(b8CompletedEntries.length).toBe(15);
      for (const entry of b8CompletedEntries) {
        const entryErrors = validateCompletedRepairEntry(entry, allowlistVersions);
        allErrors.push(...entryErrors);
        if (candidateVersions.has(entry.version)) {
          allErrors.push(`Version ${entry.version}: completed but still in verification candidates`);
    ...
    // Batch 9 uses whole-file SHA
    const b9RepairPath = resolve(evidenceDir, 'batch-09-repair.json');
    if (existsSync(b9RepairPath)) {
      const b9CompletedEntries = manifest.filter(e => e.repair_batch === 9 && e.repair_status === 'completed');
      expect(b9CompletedEntries.length).toBe(4);
      for (const entry of b9CompletedEntries) {
        const entryErrors = validateCompletedRepairEntry(entry, allowlistVersions);
        allErrors.push(...entryErrors);
      }
    }
    ...
    const perEntryRepairs = repairs.filter(r => r.batch_number !== 7 && r.batch_number !== 8 && r.batch_number !== 9);
    for (const repair of perEntryRepairs) {
      ...
      expect(batchCompletedEntries.length).toBe(batchNum === 9 ? 4 : 15);
...
describe('Batch 2 repair-specific validation', () => {
  ...
  it('valid Batch 1, Batch 2, Batch 3, Batch 4, Batch 5, Batch 6, and Batch 7 repair evidence passes', () => {
    ...
    expect(new Set(allVersions).size).toBe(allVersions.length);
    expect(allVersions.length).toBe(109); // 7*15 + 4 (B9), excluding batch-06

    // Each batch has 4+ versions and all passed
    for (const r of repairs) {
      expect(r.approved_versions.length).toBeGreaterThanOrEqual(4);
      // Batch 4 uses top-level booleans instead of confirmations object
      if (r.confirmations) {
...
describe('Batch 3 verification state consistency', () => {
  ...
  it('allowlist has 4 Batch 9 entries after Batch 8 closeout', () => {
    const allowlistPath = resolve('docs/migrations/101-246-repair-allowlist.json');
...
describe('Batch 3 repair-specific validation', () => {
  ...
  it('valid Batch 1+2+3+4+5+6+7 repair evidence passes', () => {
    ...
    expect(new Set(allVersions).size).toBe(allVersions.length);
    expect(allVersions.length).toBe(124);
  ...
  it('all 105 completed batch-repaired entries in manifest have valid digests', () => {
    ...
    // Classification counts post Wave 2 verification
    const counts: Record<string, number> = {};
    manifest.forEach(e => { counts[e.current_classification] = (counts[e.current_classification] || 0) + 1; });
    expect(counts['ALIGNED_TRACKED']).toBe(132);
    expect(counts['VERIFIED_APPLIED_UNTRACKED'] || 0).toBe(0);
    ...
    expect(0 + 0 + repairedCandidates).toBe(124);
    expect(repairedCandidates).toBe(124);

    // Verify all 120 completed entries NOT in allowlist or candidates
    ...
    // Verify all Batch 1-5 digests recompute (Batches 6, 7, 8 use whole-file SHA, validated separately)
    const repairFiles = readdirSync(evidenceDir)
      .filter(f => /^batch-\d+-repair\.json$/.test(f) && f !== 'batch-06-repair.json' && f !== 'batch-07-repair.json' && f !== 'batch-08-repair.json' && f !== 'batch-09-repair.json')
      .sort();
    const allErrors: string[] = [];

    // Batch 8 uses whole-file SHA — validate its 15 entries against the file digest
    const b8RepairPath2 = resolve(evidenceDir, 'batch-08-repair.json');
    if (existsSync(b8RepairPath2)) {
      const b8Entries = manifest.filter(e => e.repair_batch === 8 && e.repair_status === 'completed');
      for (const entry of b8Entries) {
        const entryErrors = validateCompletedRepairEntry(entry, allowlistVersions);
        allErrors.push(...entryErrors);
      }
    }
...
describe('Post-Batch 5 repair allowlist validation', () => {
  ...
  it('exactly 19 VERIFIED_APPLIED_UNTRACKED entries after Wave 2', () => {
    const manifest = JSON.parse(readFileSync(resolve('docs/migrations/101-246-production-reconciliation.json'), 'utf-8'));
...
describe('Batch 5 candidate validation', () => {
  ...
  it('exactly 0 candidates after Wave 2', () => {
  ...
  it('candidate list is empty after Wave 2', () => {
    const candidates = JSON.parse(readFileSync(resolve('docs/migrations/101-246-verification-candidates.json'), 'utf-8'));
    expect(candidates).toEqual([]);
  });

  it('no remaining PENDING entries in manifest after Wave 2', () => {
    const manifest = JSON.parse(readFileSync(resolve('docs/migrations/101-246-production-reconciliation.json'), 'utf-8'));
    const pending = manifest.filter((e: any) => e.current_classification === 'PENDING_PRODUCTION_REVERIFICATION');
    expect(pending.length).toBe(0);
  });

  it('Wave 2 Batch 8 entries are ALIGNED_TRACKED and Batch 9 entries are VERIFIED', () => {
    const manifest = JSON.parse(readFileSync(resolve('docs/migrations/101-246-production-reconciliation.json'), 'utf-8'));
    const batch8Versions = ['227','228','229','230','231','232','233','234','235','236','237','238','239','240','241'];
    const batch9Versions = ['242','243','245','246'];
    for (const v of batch8Versions) {
      const me = manifest.find((e: ManifestEntry) => e.version === v);
      expect(me).toBeDefined();
      expect(me!.current_classification).toBe('ALIGNED_TRACKED');
    }
    for (const v of batch9Versions) {
      const me = manifest.find((e: ManifestEntry) => e.version === v);
      expect(me).toBeDefined();
      expect(me!.current_classification).toBe('ALIGNED_TRACKED');
    }
...
describe('Batch 5 classification counts', () => {
  it('113/19/0/12/2 and completed repairs at 113 after Wave 2 verification', () => {
    ...
    expect(completedRepairs).toBe(132);
  });
});
...
describe('Batch 5 CLI rejection tests', () => {
  ...
  it('confirms allowlist has 4 Batch 9 entries after Wave 2', () => {
    const allowlist = JSON.parse(readFileSync(resolve('docs/migrations/101-246-repair-allowlist.json'), 'utf-8'));
    expect(allowlist.length).toBe(0);
  });
...
describe('Wave 1 rejection tests', () => {
  ...
  // 13. Batch 6 version added to allowlist (must be exactly Batch 8)
  it('rejects when Batch 6 version present in allowlist', () => {
    const tmpDir = createTestFixture();
    try {
      const alPath = join(tmpDir, 'docs', 'migrations', '101-246-repair-allowlist.json');
      ...
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/Allowlist.*16.*expected 15|Allowlist versions|allowlist/i);
    } finally { cleanupFixture(tmpDir); }
  }, 30000);

  // 14. Extra version added to allowlist (must be exactly Batch 8)
  it('rejects extra version in allowlist', () => {
    const tmpDir = createTestFixture();
    try {
      const alPath = join(tmpDir, 'docs', 'migrations', '101-246-repair-allowlist.json');
      const al = JSON.parse(readFileSync(alPath, 'utf-8'));
  ...
  it('rejects when candidate list differs from exact 19 versions', () => {
    ...
    try {
      ...
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/candidate|ALIGNED.*expected 113|PENDING.*expected 0|VERIFIED.*expected 19/i);
    } finally { cleanupFixture(tmpDir); }
  }, 30000);
  ...
  it('rejects when classification totals differ from expected', () => {
    ...
    try {
      ...
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/ALIGNED.*131.*expected 132|PENDING.*1.*expected 0/i);
    } finally { cleanupFixture(tmpDir); }
  }, 30000);
...
describe('Batch 6 closeout rejection tests', () => {
  ...
  // 16. Batch 6 version added to allowlist (must be exactly Batch 8)
  ...
  it('rejects non-empty candidate list', () => {
    const tmpDir = createTestFixture();
    try {
      const cPath = join(tmpDir, 'docs', 'migrations', '101-246-verification-candidates.json');
      const c = JSON.parse(readFileSync(cPath, 'utf-8'));
      ...
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/Candidate.*1.*expected 0|Candidates.*expected/i);
    } finally { cleanupFixture(tmpDir); }
  }, 30000);
  ...
  it('rejects classification totals differing from expected', () => {
    ...
    try {
      const mPath = join(tmpDir, 'docs', 'migrations', '101-246-production-reconciliation.json');
      const m = JSON.parse(readFileSync(mPath, 'utf-8'));
      // Move a Batch 7 entry (now ALIGNED_TRACKED) to PENDING — ALIGNED drops to 112, PENDING rises to 1
      const e208 = m.find((e: any) => e.version === '208');
...
describe('Batch 7 closeout rejection tests', () => {
  ...
  it('rejects Batch 7 not ALIGNED_TRACKED', () => {
    ...
    try {
      ...
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/208.*classification|ALIGNED.*131.*expected 132/i);
    } finally { cleanupFixture(tmpDir); }
  }, 30000);
  ...
  it('rejects Batch 7 version in allowlist', () => {
    ...
    try {
      const aPath = join(tmpDir, 'docs', 'migrations', '101-246-repair-allowlist.json');
      const a = JSON.parse(readFileSync(aPath, 'utf-8'));
      // Allowlist has 15 Batch 8 entries; adding a Batch 7 version entry should be rejected
  ...
  // 16. Duplicate Batch 8 version in allowlist
  it('rejects duplicate Batch 8 version in allowlist', () => {
    const tmpDir = createTestFixture();
    try {
      const aPath = join(tmpDir, 'docs', 'migrations', '101-246-repair-allowlist.json');
      const a = JSON.parse(readFileSync(aPath, 'utf-8'));
      // Add duplicate of existing Batch 8 entry
      a.push({ version: '227', filename: '227_customer_ltv_tier.sql', checksum: 'x', classification: 'VERIFIED_APPLIED_UNTRACKED', repair_action: 'migration_history_repair_only', confidence: 'HIGH', verification_batch: 8, production_evidence_path: 'x', production_evidence_digest: 'x' });
      writeFileSync(aPath, JSON.stringify(a, null, 2) + '\n');
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/Allowlist.*16.*expected 15|Allowlist versions|allowlist/i);
  ...
  // 17. Extra entry added to allowlist
  it('rejects extra entry in allowlist', () => {
    const tmpDir = createTestFixture();
    try {
      const aPath = join(tmpDir, 'docs', 'migrations', '101-246-repair-allowlist.json');
      const a = JSON.parse(readFileSync(aPath, 'utf-8'));
  ...
  // 18. Candidate list is not empty
  it('rejects non-empty candidate list', () => {
    const tmpDir = createTestFixture();
    try {
      const cPath = join(tmpDir, 'docs', 'migrations', '101-246-verification-candidates.json');
...
// ══════════════════════════════════════════════════════════════
// WAVE 2 (BATCHES 8-9) REJECTION TESTS
// ══════════════════════════════════════════════════════════════
describe('Wave 2 rejection tests', () => {
  const { execSync } = require('child_process');
  const { mkdirSync, cpSync, rmSync, writeFileSync } = require('fs');
  const { join } = require('path');
  const os = require('os');
  ...
  function createTestFixture(): string {
    const tmpDir = join(os.tmpdir(), `wave2-test-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
    mkdirSync(join(tmpDir, 'docs', 'migrations', 'evidence'), { recursive: true });
    mkdirSync(join(tmpDir, 'supabase', 'migrations'), { recursive: true });
  ...
  // Positive baseline
  it('accepts unmodified Wave 2 evidence', () => {
    const tmpDir = createTestFixture();
    try {
      const { exitCode } = runValidatorInFixture(tmpDir);
      expect(exitCode).toBe(0);
  ...
  // 1. Missing Batch 8 evidence
  it('rejects missing Batch 8 evidence', () => {
    const tmpDir = createTestFixture();
    try {
      rmSync(join(tmpDir, 'docs', 'migrations', 'evidence', 'batch-08-production-verification.json'));
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/Batch 8 evidence file missing/i);
    } finally { cleanupFixture(tmpDir); }
  }, 30000);

  // 2. Batch 8 SHA mismatch
  it('rejects Batch 8 evidence SHA mismatch', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', 'evidence', 'batch-08-production-verification.json');
      writeFileSync(p, '{"tampered": true}\n');
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/Batch 8 SHA mismatch/i);
    } finally { cleanupFixture(tmpDir); }
  }, 30000);

  // 3. Missing Batch 9 evidence
  it('rejects missing Batch 9 evidence', () => {
    const tmpDir = createTestFixture();
    try {
      rmSync(join(tmpDir, 'docs', 'migrations', 'evidence', 'batch-09-production-verification.json'));
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/Batch 9 evidence file missing/i);
    } finally { cleanupFixture(tmpDir); }
  }, 30000);

  // 4. Batch 9 SHA mismatch
  it('rejects Batch 9 evidence SHA mismatch', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', 'evidence', 'batch-09-production-verification.json');
      writeFileSync(p, '{"tampered": true}\n');
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/Batch 9 SHA mismatch/i);
    } finally { cleanupFixture(tmpDir); }
  }, 30000);

  // 5. Wave 2 SHA mismatch
  it('rejects Wave 2 summary SHA mismatch', () => {
    const tmpDir = createTestFixture();
    try {
      ...
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/Wave 2 summary SHA mismatch/i);
    } finally { cleanupFixture(tmpDir); }
  }, 30000);

  // 6. Wave summary embeds wrong Batch 8 SHA
  it('rejects wave summary with wrong Batch 8 SHA', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', 'evidence', 'wave-02-production-verification.json');
      const d = JSON.parse(readFileSync(p, 'utf-8'));
      d.batch_08_evidence_sha256 = 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa';
      writeFileSync(p, JSON.stringify(d, null, 2) + '\n');
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
  ...
  // 7. Wrong Batch 8 version set
  it('rejects wrong Batch 8 version set', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', 'evidence', 'batch-08-production-verification.json');
      const d = JSON.parse(readFileSync(p, 'utf-8'));
  ...
  // 8. Wrong Batch 9 version set
  it('rejects wrong Batch 9 version set', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', 'evidence', 'batch-09-production-verification.json');
      const d = JSON.parse(readFileSync(p, 'utf-8'));
  ...
  // 9. Wrong migration count
  it('rejects wrong Batch 8 migration count', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', 'evidence', 'batch-08-production-verification.json');
      const d = JSON.parse(readFileSync(p, 'utf-8'));
      d.total_migrations = 99;
      writeFileSync(p, JSON.stringify(d, null, 2) + '\n');
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
  ...
  // 10. Migration digest mismatch
  it('rejects migration digest mismatch', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', 'evidence', 'batch-08-production-verification.json');
      const d = JSON.parse(readFileSync(p, 'utf-8'));
      d.migrations[0].migration_evidence_digest = 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa';
      writeFileSync(p, JSON.stringify(d, null, 2) + '\n');
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
  ...
  // 11. Failed result
  it('rejects failed comparison result', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', 'evidence', 'batch-08-production-verification.json');
      const d = JSON.parse(readFileSync(p, 'utf-8'));
  ...
  // 12. Equivalent-stricter lineage changed
  it('rejects equivalent-stricter lineage change in Migration 233', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', 'evidence', 'batch-08-production-verification.json');
      const d = JSON.parse(readFileSync(p, 'utf-8'));
      ...
      eqObj.equivalent_stricter_by = 'migration_999';
      writeFileSync(p, JSON.stringify(d, null, 2) + '\n');
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
  ...
  // 13. Superseded lineage changed
  it('rejects superseded lineage change in Migration 229', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', 'evidence', 'batch-08-production-verification.json');
      const d = JSON.parse(readFileSync(p, 'utf-8'));
      ...
      supObj.superseded_by = 'migration_999';
      writeFileSync(p, JSON.stringify(d, null, 2) + '\n');
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
  ...
  // 14. Pre/post snapshots differ
  it('rejects differing pre/post snapshots', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', 'evidence', 'batch-08-production-verification.json');
      const d = JSON.parse(readFileSync(p, 'utf-8'));
      d.post_verification_ordered_snapshot = ['changed'];
      writeFileSync(p, JSON.stringify(d, null, 2) + '\n');
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
  ...
  // 15. Occurrence value nonzero
  it('rejects nonzero occurrence value', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', 'evidence', 'batch-08-production-verification.json');
      const d = JSON.parse(readFileSync(p, 'utf-8'));
      d.pre_verification_occurrence_map['227'] = 1;
      writeFileSync(p, JSON.stringify(d, null, 2) + '\n');
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
  ...
  // 16. Safety field false
  it('rejects safety confirmation set to false', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', 'evidence', 'batch-08-production-verification.json');
      const d = JSON.parse(readFileSync(p, 'utf-8'));
      d.safety_confirmations.all_queries_read_only = false;
      writeFileSync(p, JSON.stringify(d, null, 2) + '\n');
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
  ...
  // 17. Nonempty allowlist rejected (all repairs complete)
  it('rejects nonempty allowlist', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', '101-246-repair-allowlist.json');
      writeFileSync(p, JSON.stringify([{version:'242'}], null, 2) + '\n');
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/Allowlist|approved|1.*expected 0/i);
    } finally { cleanupFixture(tmpDir); }
  }, 30000);

  // 18. Batch 9 version added to allowlist
  it('rejects Batch 9 version added to allowlist', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', '101-246-repair-allowlist.json');
      const d = JSON.parse(readFileSync(p, 'utf-8'));
      d.push({
        version: '242',
        filename: '242_growth_credit_atomic.sql',
        checksum: 'x',
        classification: 'VERIFIED_APPLIED_UNTRACKED',
        repair_action: 'migration_history_repair_only',
        confidence: 'HIGH',
        verification_batch: 9,
        production_evidence_path: 'x',
        production_evidence_digest: 'x'
      });
      writeFileSync(p, JSON.stringify(d, null, 2) + '\n');
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/Allowlist.*5.*expected 4|allowlist.*approved/i);
    } finally { cleanupFixture(tmpDir); }
  }, 30000);

  // 19. Batch 9 incorrectly marked as completed/tracked
  it('rejects Batch 9 version reverted to VERIFIED_APPLIED_UNTRACKED', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', '101-246-production-reconciliation.json');
      const d = JSON.parse(readFileSync(p, 'utf-8'));
      const entry = d.find((e: any) => e.version === '242');
      entry.current_classification = 'VERIFIED_APPLIED_UNTRACKED';
      entry.remote_tracked = false;
      entry.repair_status = 'approved_for_repair';
      writeFileSync(p, JSON.stringify(d, null, 2) + '\n');
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/ALIGNED.*131.*expected 132|VERIFIED.*1.*expected 0/i);
    } finally { cleanupFixture(tmpDir); }
  }, 30000);

  // 20. Candidate registry not empty
  it('rejects non-empty candidate registry', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', '101-246-verification-candidates.json');
      const d = JSON.parse(readFileSync(p, 'utf-8'));
      d.push({
        version: '227',
        filename: '227_customer_ltv_tier.sql',
        checksum: 'x',
        expected_object_digest: 'x',
        expected_object_count: 2
      });
      writeFileSync(p, JSON.stringify(d, null, 2) + '\n');
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/Candidate.*1.*expected 0|Candidates.*expected.*PENDING/i);
    } finally { cleanupFixture(tmpDir); }
  }, 30000);

  // 21. Classification totals differ
  it('rejects changed classification totals', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', '101-246-production-reconciliation.json');
      const d = JSON.parse(readFileSync(p, 'utf-8'));
      const entry = d.find((e: any) => e.version === '227');
      entry.current_classification = 'PENDING_PRODUCTION_REVERIFICATION';
      writeFileSync(p, JSON.stringify(d, null, 2) + '\n');
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/VERIFIED.*1.*expected 0|PENDING.*1.*expected 0|ALIGNED.*expected/i);
    } finally { cleanupFixture(tmpDir); }
  }, 30000);

  // 22. Completed repair count differs
  it('rejects changed completed repair count', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', '101-246-production-reconciliation.json');
      const d = JSON.parse(readFileSync(p, 'utf-8'));
      const entry = d.find((e: any) =>
        e.current_classification === 'ALIGNED_TRACKED' &&
        e.repair_status === 'completed' &&
        e.original_classification === 'VERIFIED_APPLIED_UNTRACKED'
      );
      ...
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/repair.*count|Completed.*104.*expected 105|repaired.*candidate/i);
    } finally { cleanupFixture(tmpDir); }
  }, 30000);

  // 23. NOT_VERIFIABLE_SAFELY classification changed
  it('rejects NOT_VERIFIABLE_SAFELY classification change', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', '101-246-production-reconciliation.json');
      const d = JSON.parse(readFileSync(p, 'utf-8'));
      const entry = d.find((e: any) => e.version === '101');
      entry.current_classification = 'VERIFIED_APPLIED_UNTRACKED';
      writeFileSync(p, JSON.stringify(d, null, 2) + '\n');
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/NOT_VERIFIABLE.*11.*expected 12|NV.*classification changed|VERIFIED.*1.*expected 0/i);
    } finally { cleanupFixture(tmpDir); }
  }, 30000);

  // 24. Existing superseded classification changed
  it('rejects SUPERSEDED classification change', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', '101-246-production-reconciliation.json');
      const d = JSON.parse(readFileSync(p, 'utf-8'));
      const entry = d.find((e: any) => e.version === '122');
      entry.current_classification = 'VERIFIED_APPLIED_UNTRACKED';
      writeFileSync(p, JSON.stringify(d, null, 2) + '\n');
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/SUPERSEDED.*1.*expected 2|Superseded.*classification changed|VERIFIED.*1.*expected 0/i);
    } finally { cleanupFixture(tmpDir); }
  }, 30000);
...
// ══════════════════════════════════════════════════════════════
// BATCH 8 REPAIR CLOSEOUT REJECTION TESTS
// ══════════════════════════════════════════════════════════════
describe('Batch 8 closeout rejection tests', () => {
  const { execSync } = require('child_process');
  const { mkdirSync, cpSync, rmSync, writeFileSync } = require('fs');
  const { join } = require('path');
  const os = require('os');
  ...
  function createTestFixture(): string {
    const tmpDir = join(os.tmpdir(), `b8close-test-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
    mkdirSync(join(tmpDir, 'docs', 'migrations', 'evidence'), { recursive: true });
    mkdirSync(join(tmpDir, 'supabase', 'migrations'), { recursive: true });
    cpSync(join(SRC_DOCS_MIGRATIONS, '101-246-production-reconciliation.json'), join(tmpDir, 'docs', 'migrations', '101-246-production-reconciliation.json'));
    cpSync(join(SRC_DOCS_MIGRATIONS, '101-246-repair-allowlist.json'), join(tmpDir, 'docs', 'migrations', '101-246-repair-allowlist.json'));
  ...
  // Positive baseline
  it('accepts valid Batch 8 closeout state', () => {
    const tmpDir = createTestFixture();
    try {
      const { exitCode } = runValidatorInFixture(tmpDir);
      expect(exitCode).toBe(0);
  ...
  // 1. Missing Batch 8 repair evidence
  it('rejects missing Batch 8 repair evidence', () => {
    const tmpDir = createTestFixture();
    try {
      rmSync(join(tmpDir, 'docs', 'migrations', 'evidence', 'batch-08-repair.json'));
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/Batch 8 repair evidence.*missing/i);
    } finally { cleanupFixture(tmpDir); }
  }, 30000);

  // 2. Repair evidence SHA mismatch
  it('rejects Batch 8 repair evidence SHA mismatch', () => {
    const tmpDir = createTestFixture();
    try {
      ...
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/Batch 8 repair evidence SHA mismatch/i);
    } finally { cleanupFixture(tmpDir); }
  }, 30000);

  // 3. Batch 8 entry still repair_eligible
  it('rejects Batch 8 entry still marked repair_eligible', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', '101-246-production-reconciliation.json');
      const d = JSON.parse(readFileSync(p, 'utf-8'));
      const entry = d.find((e: any) => e.version === '227');
      entry.repair_eligible = true;
      entry.repair_status = 'approved_for_repair';
      entry.current_classification = 'VERIFIED_APPLIED_UNTRACKED';
      entry.remote_tracked = false;
      writeFileSync(p, JSON.stringify(d, null, 2) + '\n');
  ...
  // 4. Batch 8 entry not ALIGNED_TRACKED
  it('rejects Batch 8 entry not ALIGNED_TRACKED', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', '101-246-production-reconciliation.json');
      const d = JSON.parse(readFileSync(p, 'utf-8'));
      ...
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/ALIGNED.*131.*expected 132|B8 227 classification/i);
    } finally { cleanupFixture(tmpDir); }
  }, 30000);

  // 5. Batch 9 not activated
  it('rejects Batch 9 entry not activated for repair', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', '101-246-production-reconciliation.json');
      const d = JSON.parse(readFileSync(p, 'utf-8'));
      const entry = d.find((e: any) => e.version === '242');
      entry.repair_eligible = false;
      entry.repair_status = 'verified_waiting_for_activation';
      writeFileSync(p, JSON.stringify(d, null, 2) + '\n');
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/B9 242 repair_eligible|B9 242 repair_status|approved/i);
    } finally { cleanupFixture(tmpDir); }
  }, 30000);

  // 6. Candidate registry nonempty
  it('rejects nonempty candidate registry', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', '101-246-verification-candidates.json');
      writeFileSync(p, JSON.stringify([{version: '227'}], null, 2) + '\n');
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/Candidates|candidate/i);
    } finally { cleanupFixture(tmpDir); }
  }, 30000);

  // 7. Classification totals differ
  it('rejects wrong classification totals', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', '101-246-production-reconciliation.json');
      const d = JSON.parse(readFileSync(p, 'utf-8'));
  ...
  // 8. Completed repairs not 120
  it('rejects completed repairs not 120', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', '101-246-production-reconciliation.json');
      const d = JSON.parse(readFileSync(p, 'utf-8'));
      ...
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/candidate|cohort|123|124/i);
    } finally { cleanupFixture(tmpDir); }
  }, 30000);
  ...
  // 31. Superseded: Batch 9 is now completed and remote_tracked=true

  // 32-38. Superseded by Batch 9 final closeout tests (B9 is now ALIGNED_TRACKED, allowlist is empty)
  ...
  it('rejects change to SUPERSEDED classification', () => {
    ...
    try {
      ...
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/SUPERSEDED.*1.*expected 2|ALIGNED.*133.*expected 132/i);
    } finally { cleanupFixture(tmpDir); }
  }, 30000);

  // 41-42. Superseded by Batch 9 final closeout (allowlist is now empty)
...
// ══════════════════════════════════════════════════════════════
// BATCH 9 FINAL CLOSEOUT TESTS
// ══════════════════════════════════════════════════════════════
describe('Batch 9 final closeout tests', () => {
  const { execSync } = require('child_process');
  const { mkdirSync, cpSync, rmSync, writeFileSync } = require('fs');
  const { join } = require('path');
  const os = require('os');
  ...
  function createTestFixture(): string {
    const tmpDir = join(os.tmpdir(), `b9close-test-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
    mkdirSync(join(tmpDir, 'docs', 'migrations', 'evidence'), { recursive: true });
    mkdirSync(join(tmpDir, 'supabase', 'migrations'), { recursive: true });
    cpSync(join(SRC_DOCS_MIGRATIONS, '101-246-production-reconciliation.json'), join(tmpDir, 'docs', 'migrations', '101-246-production-reconciliation.json'));
    cpSync(join(SRC_DOCS_MIGRATIONS, '101-246-repair-allowlist.json'), join(tmpDir, 'docs', 'migrations', '101-246-repair-allowlist.json'));
  ...
  it('accepts valid Batch 9 final closeout state', () => {
    const tmpDir = createTestFixture();
    try {
      const { exitCode } = runValidatorInFixture(tmpDir);
      expect(exitCode).toBe(0);
  ...
  it('rejects missing Batch 9 repair evidence', () => {
    const tmpDir = createTestFixture();
    try {
      rmSync(join(tmpDir, 'docs', 'migrations', 'evidence', 'batch-09-repair.json'));
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/Batch 9 repair evidence.*missing/i);
    } finally { cleanupFixture(tmpDir); }
  }, 30000);

  it('rejects Batch 9 repair evidence SHA mismatch', () => {
    const tmpDir = createTestFixture();
    try {
      ...
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/Batch 9 repair evidence SHA mismatch/i);
    } finally { cleanupFixture(tmpDir); }
  }, 30000);

  it('rejects Batch 9 entry not ALIGNED_TRACKED', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', '101-246-production-reconciliation.json');
      const d = JSON.parse(readFileSync(p, 'utf-8'));
      d.find((e: any) => e.version === '242').current_classification = 'VERIFIED_APPLIED_UNTRACKED';
      writeFileSync(p, JSON.stringify(d, null, 2) + '\n');
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/ALIGNED.*131.*expected 132|VERIFIED.*1.*expected 0/i);
  ...
  it('rejects nonempty allowlist after final closeout', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', '101-246-repair-allowlist.json');
      writeFileSync(p, JSON.stringify([{version: '242'}], null, 2) + '\n');
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/Allowlist.*1.*expected 0|allowlist|approved/i);
    } finally { cleanupFixture(tmpDir); }
  }, 30000);

  it('rejects completed repairs not 124', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', '101-246-production-reconciliation.json');
      const d = JSON.parse(readFileSync(p, 'utf-8'));
      d.find((e: any) => e.version === '242').original_classification = 'ALIGNED_TRACKED';
      writeFileSync(p, JSON.stringify(d, null, 2) + '\n');
      const { exitCode, output } = runValidatorInFixture(tmpDir);
      expect(exitCode).not.toBe(0);
      expect(output).toMatch(/candidate|cohort|123|124/i);
  ...
  it('rejects NV classification change', () => {
    const tmpDir = createTestFixture();
    try {
      const p = join(tmpDir, 'docs', 'migrations', '101-246-production-reconciliation.json');
      const d = JSON.parse(readFileSync(p, 'utf-8'));
