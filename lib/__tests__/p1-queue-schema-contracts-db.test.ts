...
describeDb('P1-QUEUE-1: queue_entries cancelled status (Real PostgreSQL)', () => {
  ...
  beforeAll(() => {
    ...
    psql(`
      INSERT INTO businesses (id, name, owner_id, category, country_code, phone, subscription_tier)
      VALUES
        ('${BIZ_ID}', 'Queue Test Biz', '0a300000-0000-0000-0000-000000aa0001', 'salon', 'NG', '+2341111111111', 'growth'),
        ('${BIZ_ID_2}', 'Queue Test Biz 2', '0a300000-0000-0000-0000-000000aa0002', 'salon', 'NG', '+2342222222222', 'growth')
  ...
  it('3. cancelled entry is NOT counted as waiting', () => {
    ...
    `);
    expect(parseInt(waitingCount)).toBe(1); // only the waiting one
  ...
  it('4. customer can rejoin after cancellation (unique index allows it)', () => {
    ...
    // Rejoin — should succeed because unique index only covers waiting/serving
    psql(`
      INSERT INTO queue_entries (business_id, customer_phone, queue_number, queue_date, status, channel)
    ...
    `);
    expect(parseInt(count)).toBe(2); // both cancelled and new waiting
  ...
  it('5. repeated cancellation UPDATE is idempotent (0 rows affected if already cancelled)', () => {
    ...
    // Try to cancel again — WHERE status IN ('waiting') won't match
...
describeDb('P1-QUEUE-2: queue_reopen_subscriptions (Real PostgreSQL)', () => {
  const BIZ_ID = '0a300000-0000-0000-0000-000000000001';
  const BIZ_ID_2 = '0a300000-0000-0000-0000-000000000002';
  ...
  it('12. after notifying, same customer can subscribe again', () => {
    ...
    // New subscription should succeed (unique index only covers status='waiting')
    psql(`
      INSERT INTO queue_reopen_subscriptions (business_id, customer_phone, status)
