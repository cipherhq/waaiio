...
describe.skipIf(!canRun)('M378 Provider-Neutral Subscriptions — PostgreSQL proofs', () => {
  ...
  // ══════════════════════════════════════════════════════════
  // Deterministic multi-session concurrent PostgreSQL proofs
  //
  // Each test uses an advisory-lock barrier to PROVE both sessions
  // are alive and blocked before either enters the contested operation.
  //
  // Barrier protocol:
  // 1. Main holds EXCLUSIVE advisory lock (barrier)
  // 2. Two contestant sessions request SHARED lock (block on barrier)
  // 3. Main verifies pg_locks shows 2 waiters (granted=false)
  // 4. Main releases exclusive lock
  // 5. Both sessions acquire shared lock simultaneously, release it,
  ...
  it('37. DETERMINISTIC CONCURRENT checkout claims — rendezvous-proven overlap, one pending intent', async () => {
    ...
    const concBizId = psql(`
      INSERT INTO businesses (id, name, slug, owner_id, country_code, category, address, city, neighborhood, phone)
      VALUES (gen_random_uuid(), 'DetConcClaim', 'det-conc-${Date.now()}', '${testUserId}', 'NG', 'restaurant', '100 DetConc St', 'Lagos', 'VI', '+2348044444444')
      RETURNING id::text;
    `);
  ...
  it('38. DETERMINISTIC CONCURRENT terminal replacement — rendezvous-proven overlap, one pending replacement', async () => {
    ...
    const replBizId = psql(`
      INSERT INTO businesses (id, name, slug, owner_id, country_code, category, address, city, neighborhood, phone)
      VALUES (gen_random_uuid(), 'DetConcRepl', 'det-repl-${Date.now()}', '${testUserId}', 'NG', 'restaurant', '200 DetRepl St', 'Lagos', 'VI', '+2348055555555')
      RETURNING id::text;
    `);
  ...
  it('39. DETERMINISTIC CONCURRENT duplicate renewals — rendezvous-proven overlap, one value mutation, one quarantine', async () => {
    ...
    // Exactly one success and one quarantine — no duplicate entitlement
    ...
    expect(psql(`SELECT count(*) FROM subscription_payments WHERE subscription_id='${subId}'::uuid AND provider_reference IN ('tx_det_ren_A','tx_det_ren_B') AND status='success';`)).toBe('1');

    // One quarantine record — no duplicate value
  ...
  it('86. paid_finalized evidence prevents expiry; no_evidence also prevents expiry', () => {
    ...
    // Case 2: Upgrade evidence to paid_finalized → expiry still denied
    ...
    // Evidence must be paid_finalized
  ...
  it('100. forced rollback — business UPDATE trigger failure rolls back subscription expiry', async () => {
    ...
    // Set business to growth tier BEFORE the test — proves rollback restores it
    psql(`UPDATE businesses SET subscription_tier = 'growth' WHERE id = '${bizId}'::uuid;`);
    ...
    // Verify business tier is unchanged — was 'growth' before, must still be 'growth' (NOT 'free')
    const tier = psql(`SELECT subscription_tier FROM businesses WHERE id = '${bizId}'::uuid;`);
    expect(tier).toBe('growth'); // NOT 'free' — proves rollback preserved the tier
  ...
  it('101. TRUE CONCURRENT cancellation claim — disjoint worker batches', async () => {
    // Create two eligible subscriptions for cancellation check
    const biz1 = m380Biz('t101a');
    const biz2 = m380Biz('t101b');
    const ver = currentVersion();
    ...
    // Use rendezvous barrier for concurrent claims — each worker claims batch of 1
    const bt = setupBarrierTable();
    const claimSql = `SELECT sub_id FROM claim_active_subscriptions_for_cancellation_check(1);`;

    const [s1, s2] = await Promise.all([
      psqlAsync(barrieredSql(bt, 'W1', 2, claimSql)),
      psqlAsync(barrieredSql(bt, 'W2', 2, claimSql)),
    ]);

    expect(s1.ok).toBe(true);
    expect(s2.ok).toBe(true);

    // Each worker gets exactly one row (batch size 1)
    const id1 = s1.result.split('\n').filter(Boolean).pop()!.trim();
    const id2 = s2.result.split('\n').filter(Boolean).pop()!.trim();
    expect(id1).toBeTruthy();
    expect(id2).toBeTruthy();
    // IDs must differ — disjoint claim (SKIP LOCKED ensures no overlap)
    expect(id1).not.toBe(id2);

    teardownBarrierTable(bt);
    psql(`DELETE FROM subscriptions WHERE id IN ('${sub1}'::uuid, '${sub2}'::uuid);`);
    psql(`DELETE FROM businesses WHERE id IN ('${biz1}'::uuid, '${biz2}'::uuid);`);
  }, 15000);
});
