...
|---------|------|--------|----|-------|
| 298 | `complete_order_payment_backfill` | Proposed (not deployed) | #73 | Forward fix: completes Migration 167 historical order_id backfill for 11 verified legacy rows. Hardened: orders SHARE then payments SHARE ROW EXCLUSIVE lock order, complete-row JSONB immutability snapshot, timestamp boundary, ownership guard in UPDATE, safe text comparison (no UUID casts). Real PostgreSQL tests in dedicated waaiio_m298_test database with trigger side-effect detection and URL safety guard. Guarded, idempotent, fail-closed. Updates only payments.order_id. Does not modify business_id. |
