...
|---------|------|--------|----|-------|
| 298 | `complete_order_payment_backfill` | Proposed (not deployed) | #73 | Forward fix: completes Migration 167 historical order_id backfill for 11 verified legacy rows. Hardened: no UUID casts (safe text comparison), table locks, timestamp boundary, ownership guard in UPDATE, real PostgreSQL tests. Guarded, idempotent, fail-closed. Updates only payments.order_id. Does not modify business_id. |
