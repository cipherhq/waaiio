...
PR #21 (`fix/combined-18-19-20`, head `22168545`) contains tested but unmerged financial safety work:
...
**Why stranded:**
- 100 files, 48 migrations — too large for safe review
- Migrations 244-291 conflict with main's 247-248
- Auth patterns use obsolete `profiles.role` (replaced by PR #36's `app_metadata`)

**Next action:** Extract useful protections into small, focused PRs with updated auth and renumbered migrations. Do not merge or rebase PR #21 wholesale.

## Active Next Actions

1. **Resolve Meta WABA billing** — required before AUTH-000 production verification
2. **Extract FIN-LEGACY protections** — payout idempotency, Square OAuth, credential filtering
3. **Create tracking issues** — for AUTH-001 closure and financial safety milestones
