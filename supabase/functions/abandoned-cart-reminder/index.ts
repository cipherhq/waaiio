...
async function sendWhatsApp(to: string, text: string, supabase: ReturnType<typeof createClient>, businessId: string): Promise<boolean> {
  ...
  try {
    ...
    if (bizErr || !bizCheck || bizCheck.messaging_suspended !== false) {
      log.debug(`[abandoned-cart-reminder] Send blocked: business ${businessId} suspended/unverifiable`);
      return false;
    }
    ...
    const result = await withEdgeAttemptRecording(
      supabase,
      { businessId, recipientPhone: to, phoneNumberId: whatsappPhoneId, flowType: 'abandoned-cart-reminder' },
      () => fetch(
...
Deno.serve(async () => {
  ...
  for (const session of abandonedSessions || []) {
    ...
    const sent = await sendWhatsApp(phone, msg, supabase, session.business_id);
    if (sent) {
      await supabase
        .from('bot_sessions')
