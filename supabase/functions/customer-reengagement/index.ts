...
async function sendWhatsApp(to: string, text: string, supabase: ReturnType<typeof createClient>, businessId: string): Promise<boolean> {
  ...
  try {
    ...
    const result = await withEdgeAttemptRecording(
      supabase,
      { businessId, recipientPhone: to, phoneNumberId: whatsappPhoneId, flowType: 'customer-reengagement' },
      () => fetch(
      ...
      async () => {
        ...
        if (bizErr || !bizCheck || bizCheck.messaging_suspended !== false) {
          log.debug(`[customer-reengagement] Send blocked: business ${businessId} suspended/unverifiable`);
          return false;
        }
        return true;
      },
...
Deno.serve(async () => {
  ...
  for (const customer of inactiveCustomers || []) {
    ...
    const sent = await sendWhatsApp(customer.phone, msg, supabase, customer.business_id);
    if (sent) {
      await supabase
        .from('customer_profiles')
