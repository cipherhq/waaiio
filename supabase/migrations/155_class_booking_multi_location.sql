-- Migration 155: Class booking columns + book_slot_atomic with location support
-- Classes are services with is_class=true and a repeating schedule

-- 1. Add class columns to services
ALTER TABLE services ADD COLUMN IF NOT EXISTS is_class BOOLEAN DEFAULT false;
ALTER TABLE services ADD COLUMN IF NOT EXISTS class_schedule JSONB DEFAULT '[]';
-- class_schedule format: [{"day": "monday", "time": "18:00"}, {"day": "wednesday", "time": "18:00"}]

-- 2. Recreate book_slot_atomic with p_location_id parameter
