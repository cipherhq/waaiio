...
export async function POST(request: NextRequest) {
  ...
  if (action === 'save_refs') {
    ...
    if (!country_code || !plan_refs || !expected_version_id) {
      return NextResponse.json({ error: 'Missing required fields: country_code, plan_refs, expected_version_id' }, { status: 400 });
    }
    ...
    if (countryErr || !country) {
      return NextResponse.json({ error: `Country "${country_code}" not found` }, { status: 404 });
    }
    ...
    for (const tier of ['growth', 'business'] as const) {
      ...
      if (tierPrice <= 0) {
        return NextResponse.json({ error: `No price configured for tier "${tier}" in ${country_code}` }, { status: 400 });
      }
      ...
      for (const [provider, ref] of Object.entries(tierRefs)) {
        ...
        if (provider === 'stripe') {
          return NextResponse.json({ error: 'Stripe uses inline price_data and does not require plan refs' }, { status: 400 });
        }

        if (provider === 'paystack') {
          const psKey = process.env.PAYSTACK_SECRET_KEY;
          if (!psKey) return NextResponse.json({ error: 'PAYSTACK_SECRET_KEY not configured' }, { status: 503 });
          ...
          if (!pf.ok) return NextResponse.json({ error: `Paystack ${tier} plan "${ref}" preflight failed: ${pf.reason}` }, { status: 400 });
        }

        if (provider === 'flutterwave') {
          const flwKey = process.env.FLUTTERWAVE_SECRET_KEY;
          if (!flwKey) return NextResponse.json({ error: 'FLUTTERWAVE_SECRET_KEY not configured' }, { status: 503 });
          ...
          if (!pf.ok) return NextResponse.json({ error: `Flutterwave ${tier} plan "${ref}" preflight failed: ${pf.reason}` }, { status: 400 });
        }
      }
    }
    ...
    if (error) {
      if (error.message?.includes('config_version_conflict')) {
        return NextResponse.json({ error: 'config_version_conflict', message: 'Configuration has been modified. Please reload and try again.' }, { status: 409 });
      }
  ...
  if (action === 'switch_provider') {
    ...
    if (!country_code || !new_gateway || !expected_version_id) {
      return NextResponse.json({ error: 'Missing required fields: country_code, new_gateway, expected_version_id' }, { status: 400 });
    }

    // Paystack platform lifecycle not implemented — ALWAYS deny switching TO Paystack
    if (new_gateway === 'paystack') {
      return NextResponse.json({ error: 'Paystack platform subscription lifecycle is not yet implemented. Gateway switching to Paystack is disabled.' }, { status: 400 });
    }
    ...
    if (new_gateway === 'flutterwave') {
      ...
      if (countryErr || !country) return NextResponse.json({ error: 'Country not found' }, { status: 404 });
      ...
      for (const tier of ['growth', 'business'] as const) {
        ...
        if (!flwRef) return NextResponse.json({ error: `Flutterwave plan ref not configured for tier ${tier}` }, { status: 400 });
        ...
        if (!pf.ok) return NextResponse.json({ error: `Flutterwave ${tier} plan preflight failed: ${pf.reason}` }, { status: 503 });
      }
    }
    // Stripe: verify API readiness immediately before switch
      ...
      if (!pf.ok) return NextResponse.json({ error: `Stripe readiness check failed: ${pf.reason}` }, { status: 503 });
    }
    // Unsupported gateway
    else {
      return NextResponse.json({ error: `Unsupported gateway: ${new_gateway}` }, { status: 400 });
    }
  ...
  if (action === 'update_country') {
    ...
    if (!country_code || !fields || typeof fields !== 'object') {
      return NextResponse.json({ error: 'Missing country_code or fields' }, { status: 400 });
    }
    ...
    for (const key of FORBIDDEN) {
      if (key in fields) {
        return NextResponse.json({ error: `Field "${key}" cannot be changed via generic country edit` }, { status: 400 });
      }
    }
