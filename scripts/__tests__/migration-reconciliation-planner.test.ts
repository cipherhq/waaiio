...
const { extractVersion, classifyRemoteVersions, reconcile, readLocalMigrations, ReconciliationState } = mod;

function buildLocal(vs: number[]) { return vs.map(v => ({ version: v, filename: v + '_test.sql' })); }
function buildFullLocal(extras: number[]) { return buildLocal([...Array.from({length:354},(_,i)=>i+1), ...extras]); }
function remoteThrough(n: number) { return Array.from({length:n},(_,i)=>String(i+1)); }
...
describe('readLocalMigrations', () => {
  it('reads repo', () => { const r = readLocalMigrations(); expect(r.migrations.length).toBeGreaterThan(100); expect(r.duplicates).toEqual([]); });
});
describe('reconcile', () => {
  ...
  it('clean355->no rerun', () => { const r=reconcile(FL,remoteThrough(355),SP,{intentionallyAbsent:ABSENT}); expect(r.state).toBe('READY'); expect(r.pendingMigrations.find((m:any)=>m.version===355)).toBeUndefined(); });
  it('ts355->REPAIR', () => { const r=reconcile(FL,[...remoteThrough(354),'20260902052231'],SP,{intentionallyAbsent:ABSENT,knownTimestampMappings:{'20260902052231':{repoVersion:355,schemaPresent:true}}}); expect(r.state).toBe('REPAIR_REQUIRED'); expect(r.repairSteps.some((s:string)=>s.includes('reverted'))).toBe(true); expect(r.pendingMigrations.map((m:any)=>m.version)).toEqual([356,357,359,360,361,362,363]); });
  ...
  it('schemaPresent:false->BLOCKED', () => { const r=reconcile(FL,[...remoteThrough(354),'20260902052231'],SA,{intentionallyAbsent:ABSENT,knownTimestampMappings:{'20260902052231':{repoVersion:355,schemaPresent:false}}}); expect(r.state).toBe('BLOCKED'); expect(r.errors.some((e:string)=>e.includes('NOT present'))).toBe(true); });
  ...
  it('prod->REPAIR,356-363', () => { const r=reconcile(FL,[...remoteThrough(354),'20260902052231'],SP,{intentionallyAbsent:ABSENT,knownTimestampMappings:{'20260902052231':{repoVersion:355,schemaPresent:true}}}); expect(r.state).toBe('REPAIR_REQUIRED'); expect(r.pendingMigrations.map((m:any)=>m.version)).toEqual([356,357,359,360,361,362,363]); expect(r.note).toContain('non-executable'); });
  ...
  it('358 skipped', () => { const r=reconcile(buildFullLocal([355,356,357,358,359]),remoteThrough(355),SP,{intentionallyAbsent:ABSENT}); expect(r.pendingMigrations.find((m:any)=>m.version===358)).toBeUndefined(); });
...
describe('354 regression', () => {
  it('354 items 1-354', () => { const v=remoteThrough(354); expect(v.length).toBe(354); expect(v[0]).toBe('1'); expect(v[353]).toBe('354'); });
});
describe('no write path', () => {
  it('no apply_migration', () => { expect(readFileSync(resolve('scripts/migration-reconciliation-planner.js'),'utf-8')).not.toContain('apply_migration'); });
  it('no DB writes', () => { const s=readFileSync(resolve('scripts/migration-reconciliation-planner.js'),'utf-8'); expect(s).not.toContain('INSERT INTO'); expect(s).not.toContain('createClient'); });
});
describe('CLI', () => {
  function runCLI(...a: string[]) { try { return execSync('node scripts/migration-reconciliation-planner.js '+a.join(' '),{encoding:'utf-8',timeout:15000}); } catch(e: any) { return (e.stdout||'')+'\n'+(e.stderr||''); } }
  it('refuses no files', () => { expect(runCLI()).toContain('--remote-history-file'); });
  it('prod->REPAIR_REQUIRED', () => {
    const r = readLocalMigrations(); const t354=r.migrations.filter((m:any)=>m.version<=354).map((m:any)=>String(m.version));
